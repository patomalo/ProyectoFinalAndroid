...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ...
  public void onCreate(Bundle icicle) {
    ...
    //SI NO ME EQUVIVOCO DESDE ESTA PARTE REALIZA LO Q ES SACAR FOTO, ETC,ETC
    ...
    button.setOnClickListener(new resultLayout());
    
    button = (Button) findViewById(R.id.formulasss);
    button.setOnClickListener(new formulasLayout());
  }
  
  
  public class resultLayout implements View.OnClickListener {
		public void onClick(View view) {
			Intent math =  new Intent(CaptureActivity.this, Operation.class);
			//ocrResultView.setText(" dota ");
			String prob = ocrResultView.getText().toString();
			Bundle send = new Bundle();
			send.putString("mathProb", prob);
			math.putExtras(send);
			startActivity(math);
		}
  }

  public class formulasLayout implements View.OnClickListener {
	  public void onClick(View view) {
		  Intent forms = new Intent(CaptureActivity.this, Formulas.class);
		  startActivity(forms);
	  }
  }
   ...
   * 
   * ON THIS PART
   * Esta parte es dnde se imprime las letras
   * y el texto convertido
   * despues del decode 
   * 
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    //Filter text and wrong result
    String finalResult = filterText(ocrResult.getText());
    ocrResultTextView.setText(finalResult);
    //ocrResultTextView.setText(ocrResult.getText());
    // scale betweeen 22 and 32 -- letra grande texto corto 
  ...
  String filterText(String ocrResult) {
	  Log.d(TAG, "TEXTO RECONOCIDO: "+ocrResult);
	  ocrResult = ocrResult.replace("X", "x");
	  ocrResult = ocrResult.replace("><", "x");
	  ocrResult = ocrResult.replace("Y", "y");
	  ocrResult = ocrResult.replace("S", "5");
	  ocrResult = ocrResult.replace("Z", "2");
	  ocrResult = ocrResult.replace("_", "-");
	  
	  ocrResult = ocrResult.replace("o", "0");
	  ocrResult = ocrResult.replace("O", "0");
	  ocrResult = ocrResult.replace("l", "1");
	  
	  ocrResult = ocrResult.replace("V", "y");
	  ocrResult = ocrResult.replace("v", "y");
	  
	  Log.d(TAG, "NUEVO TEXTO RECONOCIDO: "+ocrResult);
	  return ocrResult;
  }
  ...
  private boolean checkFirstLaunch() {
    try {
      ...
      if (currentVersion > lastVersion) {
        System.out.println("PATOMALO SAYS: "+currentVersion +" : "+lastVersion);
